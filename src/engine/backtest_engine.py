...
Run a small synthetic demo:
 - builds fake minute underlying + option mid
 - computes parity residual and a simple ML score
 - runs a naive quoting / fill / hedge loop
 - saves PNG plots into reports/
Run:
    python src/engine/backtest_engine.py --demo
"""
import os
import sys
import argparse
...
# Ensure project root is on sys.path so `src.theory` imports work
...
def build_synthetic(n_minutes=800):
    np.random.seed(42)
    S0 = 100.0
    mu = 0.0
    sigma_under = 0.02
    dt = 1/390
    returns = np.random.normal(mu*dt, sigma_under*np.sqrt(dt), size=n_minutes)
    ...
    df['theo_call'] = [bs_call_price(S[i], K, r, implied_vol, T_years[i]) for i in range(n_minutes)]
    df['theo_delta'] = [bs_call_delta(S[i], K, r, implied_vol, T_years[i]) for i in range(n_minutes)]

    noise_scale = 0.02
    df['mid'] = df['theo_call'] * (1 + noise_scale * np.random.normal(0, 1, n_minutes))
    vol_proxy = pd.Series(df['underlying']).pct_change().rolling(5, min_periods=1).std().fillna(0)
    df['spread'] = np.maximum(0.01, 0.02 + 5 * vol_proxy.values)

    pvK = K * np.exp(-r * df['T_years'])
    # synthetic put consistent with parity + small noise
    df['put_synthetic'] = df['mid'] - df['underlying'] + pvK + np.random.normal(0, 0.005, n_minutes) * df['mid']
...
def run_engine(df, reports_dir="reports", seed=123):
    ...
    for i, row in df.iterrows():
        ...
        size = 1.0
        if hit_ask:
            # someone buys from our ask => we sell (short) option
            executed_price = row['ask']
            option_pos -= size
            cash += executed_price * size
            # immediate hedge: target underlying = -option_pos * theo_delta
            target_underlying_pos = -option_pos * row['theo_delta']
            delta_trade = target_underlying_pos - underlying_pos
            underlying_pos += delta_trade
            cash -= delta_trade * row['underlying']
            fills += 1
        if hit_bid:
            executed_price = row['bid']
            option_pos += size
            cash -= executed_price * size
            target_underlying_pos = -option_pos * row['theo_delta']
            delta_trade = target_underlying_pos - underlying_pos
    ...
    net_pnl = df['pnl_mtm'].iloc[-1]
    returns = pd.Series(df['pnl_mtm']).diff().fillna(0)
    mean_r = returns.mean()
    std_r = returns.std() + 1e-12
    sharpe_per_min = mean_r / std_r if std_r > 0 else 0.0
    annualized_sharpe = sharpe_per_min * math.sqrt(252 * 390)

    summary = {
        'net_pnl': net_pnl,
        'annualized_sharpe_est': annualized_sharpe,
        'total_fills': fills,
        'avg_inventory': abs(df['option_pos']).mean()
    }
    ...
    # Plot 4: Parity residual vs ML score
    ...
    print(f"Saved plots to {reports_dir}/: fig_pnl.png, fig_inventory.png, fig_inventory_hist.png, fig_parity_vs_ml.png")
    return summary

def main():
    p = argparse.ArgumentParser()
    p.add_argument("--demo", action="store_true", help="run synthetic demo and save plots")
    p.add_argument("--n", type=int, default=800, help="number of minutes (synthetic)")
    args = p.parse_args()
