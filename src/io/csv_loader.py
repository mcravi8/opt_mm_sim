...
def load_option_csv(
    ...
    # optional timestamp (helps compute T precisely if expiry provided)
    ...
    if tyears_col:
        ...
        df["T_years"] = (pd.to_datetime(df["expiry"]) - df[ts_col]).dt.total_seconds() / (86400.0 * 252.0)
        df["T_years"] = df["T_years"].clip(lower=1e-6)
    else:
        # fallback: approximate 30 calendar days
    ...
    if "put_mid" in df.columns:
        pvK = df["strike"] * np.exp(-df["r"] * df["T_years"])
        out["parity_residual"] = df["call_mid"] - (df["put_mid"] + df["underlying"] - pvK - df["pv_div"])
    else:
